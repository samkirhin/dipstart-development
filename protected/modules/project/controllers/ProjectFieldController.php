...
class ProjectFieldController extends Controller
{
			...
			if ($scheme=='CSqliteSchema') {
				...
				try
				{
				    ...
				    $connection->createCommand(
				    	"CREATE TEMPORARY TABLE ".Project::model()->tableName()."_backup (".implode(',',$attr).")"
				    )->execute();
				    
				    $connection->createCommand(
				    	"INSERT INTO ".Project::model()->tableName()."_backup SELECT ".implode(',',$attr)." FROM ".Project::model()->tableName()
				    )->execute();
				    
				    $connection->createCommand(
				    	"DROP TABLE ".Project::model()->tableName()
				    )->execute();
				    
				    $connection->createCommand(
				    	"CREATE TABLE ".Project::model()->tableName()." (".implode(',',$attr).")"
				    )->execute();
				    
				    $connection->createCommand(
				    	"INSERT INTO ".Project::model()->tableName()." SELECT ".implode(',',$attr)." FROM ".Project::model()->tableName()."_backup"
				    )->execute();
				    
				    $connection->createCommand(
				    	"DROP TABLE ".Project::model()->tableName()."_backup"
				    )->execute();
	...
	public function actionAdmin() {
		$model=new ProjectField('search');
        $model->unsetAttributes();  // clear any default values
        if(isset($_GET['ProjectField']))
            $model->attributes=$_GET['ProjectField'];

        $this->render('admin',array(
            'model'=>$model,
        ));
		/*
		$dataProvider=new CActiveDataProvider('ProjectField', array(
			'pagination'=>array(
    ...
    {
        if(isset($_POST['ajax']) && $_POST['ajax']==='Project-field-form')
        {
