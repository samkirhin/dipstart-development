...
class MessageController extends Controller
{
	...
	public function actionTrash($ajax=null)
	{
		...
		Yii::app()->user->setFlash('notice', Yii::t('MailboxModule.mailbox', 'Messages in the trash are deleted within 7 days.'));
		...
		if(isset($_POST['Mailbox']['to']))
		{
			...
			if(!$conv->interlocutor_id)
			{
				...
				if(@$username) {
					...
					if(isset($_POST['ajax']['to']) && $this->module->getUserId($_POST['ajax']['to'])) {
						...
						$conv->addError('to', Yii::t('MailboxModule.mailbox', 'User not found?'));
				}
			}

			if($conv->interlocutor_id && $conv->initiator_id == $conv->interlocutor_id) {
				$conv->addError('to', Yii::t("MailboxModule.mailbox", "Can't send message to self!"));
			}
			...
			if(!$conv->hasErrors() && !$this->module->userToUser && !$this->module->isAdmin())
			{
				if(!$this->module->isAdmin($conv->to))
					$conv->addError('to', Yii::t("MailboxModule.mailbox", "Invalid user!"));
			}
	...
	public function actionReply()
	{
		...
		if(isset($_POST['text']))
		{
			...
			if($validate)
			{
				$conv->save();
				$reply->save();
				Yii::app()->user->setFlash('success', Yii::t("MailboxModule.mailbox", "Message sent!"));
				$this->redirect(array('message/inbox'));
			}
			else{
				Yii::app()->user->setFlash('error', Yii::t("MailboxModule.mailbox", "Error sending message!"));
