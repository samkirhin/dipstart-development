...
class EventsCommand extends CConsoleCommand {
	...
	const INTERVAL = 60; // »нтервал запуска скрипта в минутах
		...
		if (is_array($usersModel))
			foreach ($usersModel as $user) {
				foreach ($user->zakaz_executor as $zakaz) {
					$time = explode(':', $user->profile->notification_time); // врем€ X, за которое надо уведомл€ть (количество часов и минут), формат "5;48"
